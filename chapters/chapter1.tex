% chapter1.tex (Đã sửa lỗi cú pháp)
\chapter{Tính đơn điệu và cực trị của hàm số}
\label{ch:chapter-1}
...
% --- Phương pháp vẽ bảng biến thiên để xét tính đơn điệu ---

% Sử dụng tkz-tab để bảng biến thiên đẹp và trực quan

%\usepackage{tkz-tab} % Đảm bảo đã khai báo gói này ở preamble
...
\begin{center}
\begin{tikzpicture}
    %\tkzTabInit{<dòng 1>/<cao>, <dòng 2>/<cao>, ...}{<các mốc x>}
    \tkzTabInit{$x$/1, $y'$/1, $y$/2}{$-\infty$, $-1$, $1$, $+\infty$}
    \tkzTabLine{,+,z,-,z,+}
    \tkzTabVar{+/ $+\infty$, -/ $4$, +/ $0$, -/ $+\infty$} % Sửa giá trị y tại cực trị cho đúng
\end{tikzpicture}
\end{center}
...
\begin{lythuyetbox}{Phương pháp sử dụng bất đẳng thức để xét dấu đạo hàm}{lt:pp-bdt}
...
\begin{lythuyetbox}{Điều kiện để một điểm là cực trị}{lt:dk-cuc-tri}
\vspace{1em}
...
\begin{lythuyetbox}{Tìm cực trị của một hàm số}{lt:tim-cuc-tri}
\textbf{Các bước xét cực trị:}
\begin{enumerate}
    \item \textbf{Bước 1:} Xác định miền xác định $D$ của hàm số.
...
\begin{lythuyetbox}{Hàm trùng phương}{lt:ham-trung-phuong}
...
\begin{tikzpicture}[scale=1.1]
  ...
  % Đánh dấu các điểm
  \fill (A) circle (2pt) node[above right] {$A(0,\ 1)$};
  \fill (B) circle (2pt) node[below right] {$B(1,\ -1)$};
  \fill (C) circle (2pt) node[below left] {$C(-1,\ -1)$};
\end{tikzpicture}
\end{center}
% --- Hết hình minh họa ---
...
\begin{chuy}{Bài tập xét tính đơn điệu hàm phân thức bậc nhất/bậc nhất}{cy:bt-pt-bac-nhat}
